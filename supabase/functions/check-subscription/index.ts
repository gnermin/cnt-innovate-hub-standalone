...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      // Update member status to pending if exists
      ...
      if (member) {
        await supabaseClient
          ...
          .eq('user_id', user.id);
        logStep("Updated member status to pending");
      }
    ...
    if (hasActiveSub) {
      ...
      if (member) {
        await supabaseClient
          ...
          .eq('user_id', user.id);
        logStep("Updated member status to active");
      }
      ...
      if (member) {
        await supabaseClient
          ...
          .eq('user_id', user.id);
        logStep("Updated member status to inactive");
      }
    }
